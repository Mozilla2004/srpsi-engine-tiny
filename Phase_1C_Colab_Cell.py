# ================================================================
# Step 9: Phase 1C - Zero-Shot Extrapolation Test (T=200)
# ================================================================
#
# 目标: 验证模型在 4x 训练长度的预测能力
# 训练: 48 步 (tin=16, tout=32)
# 测试: 200 步 (4 倍训练长度)
#
# 假设:
# - Conv: 会在长程外推中崩溃 (误差爆炸)
# - Transformer: 保持稳定性 (全局注意力优势)
# - SRΨ: 中等表现 (空间不变性 vs 时间守恒性)
# ================================================================
...
print('\n🎯 Testing: 4x training length prediction (T=200)')
...
results_to_save = {
    name: {
        'short_term_avg': r['short_term_avg'],
        'long_term_avg': r['long_term_avg'],
        'extrapolation_ratio': r['extrapolation_ratio'],
        'explosion_step': r['explosion_step']
    }
    for name, r in results_zeroshot.items()
}

with open('phase_1c_results.json', 'w') as f:
    json.dump(results_to_save, f, indent=2)

print('\n💾 Results saved to phase_1c_results.json')
